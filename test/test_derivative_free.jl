...
Base.show(io::IO, results::MethodResults1) =
        ...
        for i in 1:length(f.params)
            ...
            try
                ...
                if isa(result, AbstractFloat) && isa(residual, AbstractFloat) && isfinite(residual)
                    ...
                    results.maxdifference = max(results.maxdifference, mindiff(result, f.alpha))
                end

                verbose && abs(residual) >1/10 && @printf "Large Residual [%s] %d/%d\n" f.name i p
...
if !isinteractive()
    ...
    @testset "derivative free methods" begin
        ...
        ## Newton and Halley
        Fs = [(f,b) -> find_zero((f, D(f)), b, Roots.Newton()),
    ...
    @testset "derivative free, non Float64" begin
        ...
        Ms = [Roots.Order0(), Roots.Order1(), Roots.Order1B(), Roots.Order2(), Roots.Order2B(),
              Roots.Order5(), Roots.Order8(), Roots.Order16()]
        Ts = [Float16, Float32, BigFloat]

        fn = x -> x^3 - 2x - 5
        alpha = 2.094551481542326591482386540579302963857306105628239180304128529045312189983499
        x0 = 2.0
