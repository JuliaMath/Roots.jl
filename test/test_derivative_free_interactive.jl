...
function test_function(method, F)
    ...
    for p in F.params
        ...
        try
            ...
            exact = (iszero(residual) || f(result)*f(nextfloat(result)) < 0 || f(result) * f(prevfloat(result)) < 0) ? 1 : 0
...
function identify_regressions(Dnew,Dold)
    ...
    for (k, v) in Dnew["counts"]
        ...
        sum(A[A .> 0]) < sum(A1[A1 .> 0]) && push!(out, "✓ counts decreased for function $k")
    end
...
function derivative_based_diagonostics()
    Ms = ((f,b) -> Roots.find_zero((f, D(f)), b, Roots.Newton()),
...
function COC(M, f, x0, alpha=missing)
    ...
    if ismissing(alpha)
        fs = tracks.fs
        [Float64(log(abs(fs[k+2]/fs[k+1])) /   log(abs(fs[k+1]/fs[k]))) for k in 1:length(fs)-4]
    else
        xs = tracks.xs
        es = xs .- alpha
...
function compare_convergence(Ms; F=identity)
    ...
    for (i,u) in enumerate(fns)
        ...
        for M in Ms
            ...
            @printf "%s: %s\t%.3f(-%02i)\t%.3f(-%02i)\t%.3f(-%02i)\t%.2f\t%.8f\n"  "f$i"  elide_ascii(replace(string(M), "Roots." => ""),8)  Log10(X1)... Log10(X2)... Log10(X3)... coc a
