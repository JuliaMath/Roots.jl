...
A zero-finding algorithm solves ``f(x) = 0`` for a value of ``x``. We don't discuss here *bracketing* algorithms, such as the bisection method. That method exploits the Intermediate Value Theorem to guarantee a computational zero within a bracketing interval ``[a,b]`` (one where ``f(a)`` and ``f(b)`` differ in sign. Here we discuss iterative algorithms which take one *or more* past steps to produce the next step. (That is ``x_{n+1} = F(x_n, x_{n-1}, ..., x_1, x_0)``, for some ``F`` representing the algorithm).

[Newton's Method](https://en.wikipedia.org/wiki/Newton%27s_method) is a zero-finding *iterative algorithm* easily introduced in an introductory calculus class once the concept of a *tangent line* is introduced.

The value ``x_{n+1}`` is described as the *intersection point* of the ``x``-axis with the tangent line through ``(x_n, f(x_n)``. To be explicit, we substitute ``(x_{n+1},0)`` in to the tangent line equation ``y = f(x_n) + f'(x_n)\cdot(x-x_n)``:
...
The function ``f(x) = x^5 - x - 1`` does not have a readily available closed-form solution, it being a fifth-degree polynomial, however, a graph, or other means, can show it has one zero between ``1`` and ``2``, closer to ``1``. Starting with ``x_0=1.4``, we get a visual of ``x_1`` as follows:
...
annotate!([(x0,0,"x0", :bottom), (x1, 0, "x1", :bottom), (α, 0, "α", :bottom)])
...
This last value, `c`, is also computed in the `(3,0)` method of the `LithBoonkkampIJzerman` algorithm:

```@example geometry
x3 = Roots.lmm(Roots.LithBoonkkampIJzerman{3,0}(), xs, f.(xs)) # 1.1705712924279839
```
...
xs = [x0, x1, x2]
scatter!([x0, x1, x2, x3], [0, 0, 0, 0]; markercolor=:blue)
annotate!([(x0,0,"x0", :bottom), (x1, 0, "x1", :bottom), (x2,0,"x2", :bottom),  (x3,0,"x3", :bottom)])
...
Inverse cubic is similar, we have 4 past points used to determine the next one. In this example, we solve the resulting linear system of equations numerically:
...
The latter simply says that the constrained line has slope ``f'(x_n)`` and goes through the point ``(x_n, f(x_n))``, so is the tangent line,  and then Newton's method follows.
...
Assuming the expression goes through the point ``(x_n, f(x_n))`` and the tangency conditions: ``y'(x) = f'(x)`` **and** ``y''(x) = f''(x)``, we get

The second-order Taylor polynomial ``y(x) = f(x_n) + f'(x_n)(x-x_n) + f''(x_n)/2 \cdot(x-x_n)^2`` is the solution.
...
Then the intersection with the ``x``-axis can be represented as:

```math
x_{n+1} = x_n - \frac{2}{1 + \sqrt{1 - 2L_f(x_n)}} \frac{f(x_n)}{f'(x_n)}.
```

This is known as Euler's method or the irrational Halley method:
...
The general form of a quadratic ``Ax^2 + By^2 + Cxy + Dx + Ey + F = 0`` is specialized above by setting ``B=C=0`` and ``E=1`` and then imposing the point ``(x_n, f(x_n))`` as a solution along with tangency conditions. Halley's method can be seen as the specialization to a hyperbola: ``axy + y + bx + c = 0``. This yields the curve
...
This can be similarly visualized with:
...
As there are ``4`` unknown and only ``3`` constraints, the solution will depend on a parameter, they call ``b_n,`` yielding:
...
```math
x_{n+1} = x_n - \left(1 + \frac{1}{2}\frac{L_f(x_n)}{1 + b_n \cdot (f(x_n)/f'(x_n))}\right) \frac{f(x_n)}{f'(x_n)}.
```

Newton's method is found by letting ``b_n \rightarrow 0``, Chebyshev's method is when ``b_n=0``, Halley's method is when ``b_n = -f''(x_n)/(2f'(x_n))``.
